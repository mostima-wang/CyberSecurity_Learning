- [Javascript](#javascript)
  ...
  - [JS的压缩与混淆](#js的压缩与混淆)
    ...
    - [反混淆](#反混淆)
  - [最佳实践](#最佳实践)
      - [1. 避免仅依赖客户端验证](#1-避免仅依赖客户端验证)
      - [2. 避免添加不受信任的库](#2-避免添加不受信任的库)
      - [3. 避免硬编码敏感信息（Secrets）](#3-避免硬编码敏感信息secrets)
    - [4. 压缩与混淆你的 JavaScript 代码](#4-压缩与混淆你的-javascript-代码)
- [SQL](#sql)
    - [关系型数据库 (Relational Databases)](#关系型数据库-relational-databases)
    - [非关系型数据库 (Non-relational Databases)](#非关系型数据库-non-relational-databases)
  - [常用命令](#常用命令)
    - [数据库操作命令](#数据库操作命令)
    - [表操作命令](#表操作命令)
    - [CRUD 操作命令](#crud-操作命令)
      - [1. 创建操作 (INSERT)](#1-创建操作-insert)
      - [2. 读取操作 (SELECT)](#2-读取操作-select)
      - [3. 更新操作 (UPDATE)](#3-更新操作-update)
      - [4. 删除操作 (DELETE)](#4-删除操作-delete)
    - [SQL 高级查询子句模板](#sql-高级查询子句模板)
    - [逻辑运算符](#逻辑运算符)
    - [比较运算符](#比较运算符)
    - [字符串函数](#字符串函数)
    - [聚合函数](#聚合函数)
- [Burpsuite（BP）](#burpsuitebp)
    - [其中的核心功能：](#其中的核心功能)
    - [快捷键](#快捷键)
  - [Proxy 代理](#proxy-代理)
    - [关于 Burp Proxy 必须掌握的关键点](#关于-burp-proxy-必须掌握的关键点)
    - [代理设置中的一些重要设置](#代理设置中的一些重要设置)
  - [Target（目标）](#target目标)
